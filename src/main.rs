...
fn main() -> std::io::Result<()> {
    let matches = App::new("Cargo-todo")
        ...
        .arg(Arg::with_name("todo").index(1).hidden(true))
        .arg(
    ...
    if let Some(_matches) = matches.subcommand_matches("legacy") {
        ...
        parsers.push(Parser::new(
            String::from("//todo"),
            Box::from(|x: Vec<char>| return x.last().unwrap() == &'\n'),
        ));
        ...
        parsers.push(Parser::new_callback(
            String::from("todo!("),
            Box::from(|x: Vec<char>| return x.last().unwrap() == &')'),
            _todo_macro_callback,
        ));
            ...
            P: AsRef<Path>,
        {
            let file = match File::open(&filename) {
                ...
                Err(_) => {
                    println!(
                        "{}",
                        "File '~/.cargo/todo_config' not found, creating it".red()
                    );
