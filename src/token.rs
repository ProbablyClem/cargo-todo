...
impl Token {
    ...
    pub fn inline(&self) {
        ...
        s = string_format!(
            "{} line: {} {} ".to_string(),
            self.file.clone(),
            self.line.to_string().green().to_string(),
        ...
        if self.member.is_some() {
            s = string_format!(
                "{} Member: {}".to_string(),
                s,
                self.member.clone().unwrap().red().to_string()
            );
        }
        if self.priority.is_some() {
            s = string_format!(
                "{} Priority: {}".to_string(),
                s,
                self.priority.clone().unwrap().red().to_string()
            );
        }
        if self.date.is_some() {
            s = string_format!(
                "{} Deadline: {}".to_string(),
                s,
                self.date.unwrap().to_string().red().to_string()
            );
        }
        if self.comment.is_some() {
            s = string_format!(
                "{} {}".to_string(),
                s,
                self.comment.clone().unwrap().blue().to_string()
            );
        }
        println!("{}", s);
    }
    ...
    fn add_to_comment(&mut self, s: &str) {
        if self.comment.is_none() {
            ...
            self.comment = Some(string_format!(
                "{} {}".to_string(),
